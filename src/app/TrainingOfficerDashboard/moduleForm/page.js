...
/**
 * The module form is now the ADD MODULE tab on the dashboard, not a page of its
 * own. This route is kept so existing links and bookmarks still work: it sends
 * the officer to the dashboard with that tab open.
 */
export default function ModuleFormPage() {
  const router = useRouter();

  useEffect(() => {
    router.replace("/TrainingOfficerDashboard?tab=add");
  }, [router]);
